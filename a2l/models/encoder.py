...
class MaskedLogTransformer(torch.nn.Module):
    # A Transformer-based encoder for abnormally detection on logs
    def __init__(self, num_class, vocab_size, hidden_size, num_layer, num_head,
                 dropout, decoder_hidden_size, max_len, name='LogTransformer'):
        super(MaskedLogTransformer, self).__init__()
    ...
    def forward(self, inputs, labels):
        # forward step
        outputs = self._forward(inputs)
        outputs = torch.reshape(outputs, (-1, self.num_class, inputs.shape[-1]))
        ...
        # compute metrics
        outputs = F.softmax(outputs, dim=1)
        acc = to_cuda(Accuracy(multiclass=True))(outputs, labels)
        f1 = to_cuda(F1(num_classes=self.num_class,
                        multiclass=True,
                        mdmc_average='samplewise'))(outputs, labels)
